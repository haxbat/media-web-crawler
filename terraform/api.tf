...
########### GET /get-last-execution ###########

resource "aws_api_gateway_resource" "get-last-execution" {
  rest_api_id = "${aws_api_gateway_rest_api.web-crawler.id}"
  parent_id   = "${aws_api_gateway_rest_api.web-crawler.root_resource_id}"
  path_part   = "get-last-execution"
}

resource "aws_api_gateway_method" "get-last-execution-get" {
  rest_api_id   = "${aws_api_gateway_rest_api.web-crawler.id}"
  resource_id   = "${aws_api_gateway_resource.get-last-execution.id}"
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "get-last-execution" {
  rest_api_id             = "${aws_api_gateway_rest_api.web-crawler.id}"
  resource_id             = "${aws_api_gateway_resource.get-last-execution.id}"
  http_method             = "${aws_api_gateway_method.get-last-execution-get.http_method}"
  integration_http_method = "POST"
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${aws_lambda_function.get-last-execution.arn}/invocations"
  credentials             = "arn:aws:iam::${var.account_id}:role/${aws_iam_role.web-crawler.name}"
}

resource "aws_api_gateway_method_response" "get-last-execution-get-200" {
  rest_api_id = "${aws_api_gateway_rest_api.web-crawler.id}"
  resource_id = "${aws_api_gateway_resource.get-last-execution.id}"
  http_method = "${aws_api_gateway_method.get-last-execution-get.http_method}"
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "get-last-execution-get-200-response" {
  depends_on  = [ "aws_api_gateway_integration.get-last-execution" ]
  rest_api_id = "${aws_api_gateway_rest_api.web-crawler.id}"
  resource_id = "${aws_api_gateway_resource.get-last-execution.id}"
  http_method = "${aws_api_gateway_method.get-last-execution-get.http_method}"
  status_code = "${aws_api_gateway_method_response.get-last-execution-get-200.status_code}"
}
...
resource "aws_api_gateway_deployment" "web-crawler" {
  depends_on = [
    ...
    "aws_api_gateway_method.get-last-execution-get",
    "aws_api_gateway_integration.get-last-execution"
  ]
  rest_api_id = "${aws_api_gateway_rest_api.web-crawler.id}"
